...
#: The font the shipped atlases are built from, and the only one whose terms
#: this script can state.
DEJAVU_NAME = 'DejaVu Sans Mono'
...
_UNVERIFIED_NOTICE = """
Font: %s
License: not been recorded here.

This atlas embeds the glyph images of the font above. Verify that its licence
permits embedding and redistribution before shipping this module, and replace
this notice with the terms it is actually under.
"""


def license_notice(font_name):
    """The licence text for an atlas built from ``font_name``.

    The notice travels with the glyph images, so it has to describe the font
    those images came from. Only DejaVu's terms are known here; any other font
    gets a notice saying so rather than inheriting them.
    """
    if font_name == DEJAVU_NAME:
        return _DEJAVU_NOTICE
    return _UNVERIFIED_NOTICE % (font_name,)
...
def get_atlas(size: int) -> Optional[ModuleType]:
...
def main(argv=None):
    ...
    # Find font
    font_path = args.font
    font_name = DEJAVU_NAME

    if font_path is None:
        font_path = find_dejavu_font()
        if font_path:
            print(f"Found {DEJAVU_NAME}: {font_path}")
        else:
            # No substitute is chosen here. The atlas embeds the font's glyph
            # images and carries a licence notice describing them, so quietly
            # building from whatever monospace font this machine has would
            # produce an atlas labelled with the wrong terms -- and some of the
            # fonts a search would reach are not redistributable at all.
            print(f"Error: {DEJAVU_NAME} not found, and no substitute is chosen "
                  "for you.")
            print("Install the DejaVu fonts (Debian/Ubuntu: fonts-dejavu-core), "
                  "or pass --font explicitly and replace the licence notice in "
                  "the generated modules with that font's own terms.")
            sys.exit(1)
    else:
