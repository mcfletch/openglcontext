...
class TestContext( BaseContext ):
    ...
    def OnInit( self ):
        ...
        Everything it needs to do that, it declares: the per-vertex 
        values it reads come in as "in" variables, and the values that 
        are the same for the whole draw come in as "uniform" variables.
        Here that is one of each -- a vertex position, and the matrix 
        to transform it by.
        
        Most OpenGL programs use a perspective projection matrix to 
        transform the model-space coordinates of a cartesian model into 
        the "clip coordinate" space the hardware rasterizes in.  You 
        calculate that matrix yourself, or have a library calculate it 
        for you; OpenGLContext has already worked out a perspective 
        camera sitting 10 units from the origin, and hands it to 
        Render() as mode.matrix and mode.projection.  We multiply the 
        two together and upload the result as our "modelViewProjection" 
        uniform, and the shader multiplies each vertex by it.
        
        The main() function is defined using the C-like GLSL syntax to 
        return nothing (void).  gl_Position is the one name the shader 
        does not declare: it is where the hardware reads the answer from.
        
        The "#version 330 core" line says which GLSL this is written in.
        330 is the version that ships with OpenGL 3.3, which every 
        ...
        determine what colour the fragment should be.  The shader can 
        also write different colours to different colour buffers, or 
        change the depth of the fragment, but its primary job is simply 
        to determine the colour of the pixel (a vec4() value).
        
        It declares an "out" variable to write that colour to.  A 
        fragment shader with one output writes to colour buffer 0, which 
        is the screen, so the name is ours to choose; here it is 
        fragColor.
        
        In our code here, we create a new colour for each pixel, which 
        is a pure green opaque (the last 1) value, assign it to 
        fragColor, and we are finished.
        '''
        ...
        '''We ask the program where each of the two names we declared
        lives, so that we can put values there when we draw.'''
        ...
        A VBO holds the numbers; a *vertex array object* holds the
        description of how to read them -- which buffer each attribute
        comes from, how many components it has, how far apart
        consecutive records are.  It is the only place OpenGL keeps that
        description, so we create one now and bind it whenever we
        describe or draw our geometry.
        '''
    ...
    def Render( self, mode):
        ...
        '''The shader wants a matrix to transform each vertex by.  The
        rendering pass hands us the camera it is drawing with as
        mode.matrix (where the model sits relative to the eye) and
        mode.projection (how the eye maps onto the screen); multiplying
        them gives the one matrix that takes a model-space vertex all
        the way to clip space.

        The GL_FALSE says the four-by-four we are handing over is
        already laid out the way OpenGL wants to read it.
        '''
        glUniformMatrix4fv(
        ...
        try:
            ...
            try:
                ...
                glVertexAttribPointer says how to read one attribute
                out of the bound buffer: which location it feeds, how
                many components it has (3), what type they are
                (GL_FLOAT), whether integers should be scaled into 0..1
                (GL_FALSE), how many bytes apart consecutive records are
                (0 meaning "tightly packed"), and where in the buffer to
                start.  Our array holds nothing but positions, so all of
                that is as simple as it gets; the next tutorial packs
                two attributes into one buffer and has to be more
                specific.

                glEnableVertexAttribArray then says that this location
                really is fed from an array, rather than holding one
                fixed value for every vertex.
                '''
                glEnableVertexAttribArray( self.Vertex_position_loc )
                glVertexAttribPointer(
                    self.Vertex_position_loc, 3, GL_FLOAT, GL_FALSE, 0,
                    self.vbo
                )
                ...
                the OpenGL environment: we unbind our VBO and our vertex
                array object, and unbind our shader, so that whatever
                draws next starts from a known state rather than
                inheriting ours.
                '''
            finally:
                self.vbo.unbind()
...
Terms:
    ...
    core profile -- the OpenGL API as it stands once the
        fixed-function pipeline is left out.  There is one way to draw:
        put your data in buffers, describe it in a vertex array object,
        and write the shaders that turn it into pixels.  This is what
        these tutorials use, and what OpenGLContext asks for by default.

    compatibility profile -- the same API with the older,
        fixed-function drawing model still in it: a matrix stack, a
        fixed lighting model, glBegin/glEnd, and a large number of
        global state values in place of shaders.  Drivers still offer
        it, and a program that wants it says so with
        "profile = 'compatibility'" on its context.
'''
