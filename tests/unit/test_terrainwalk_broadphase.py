...
def test_grid_query_matches_full_scan_over_random_points():
    ...
    for seed in range(20):
        ...
        w = _Walker()
        w.init_walk(None, pos, rad)
        rng = np.random.default_rng(1000 + seed)
        for _ in range(200):
            x = float(rng.uniform(-45, 45))
            z = float(rng.uniform(-45, 45))
            got = w._push_out(x, z)
...
def test_walker_inside_trunk_pushed_to_reference_position():
    ...
    for seed in range(30):
        ...
        z = tz
        w = _Walker(_FakePlatform((x, 0.0, z)))
        w.init_walk(hf, pos, rad)
        w.collide_and_clamp()
        gx, gz = float(w.platform.position[0]), float(w.platform.position[2])

        ref = _Walker(_FakePlatform((x, 0.0, z)))
        ref.init_walk(hf, pos, rad)
...
def test_dead_centre_ejects_along_x_like_reference():
    hf = HeightField(np.zeros((16, 16)), 200.0, 10.0)
    trunk = np.array([[3.0, 0.0, -2.0]], np.float32)
    rad = np.array([0.5], np.float32)
    w = _Walker()
...
def test_uniform_radius_field_matches_reference():
    ...
    pos[:, 0] = rng.uniform(-20, 20, 120)
    pos[:, 2] = rng.uniform(-20, 20, 120)
    w = _Walker()
    w.init_walk(None, pos, None)
    assert w._tw_crad is None
    for _ in range(300):
        x = float(rng.uniform(-22, 22))
        z = float(rng.uniform(-22, 22))
