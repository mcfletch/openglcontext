...
#: table offers, which is what a teapot with no ``steps`` of its own uses for
#: instancing and bounding.
FINE = teapot_nurbs.steps_for_level(0)


def _reset():
    # Per-sampling caches: {steps: (base, lid)} etc.
    Teapot._arrays = {}
...
class TestTessellateRetry:
    ...
    def test_transient_failure_then_success_retries(self, monkeypatch):
        ...
        assert Teapot._ensure_tessellated(FINE) is False     # first attempt fails
        assert FINE not in Teapot._arrays                    # NOT latched
        assert Teapot._ensure_tessellated(FINE) is True      # retried, succeeded
        assert Teapot._arrays[FINE][0] is not None
    ...
    def test_permanent_failure_latches_after_max(self, monkeypatch):
        ...
        for _ in range(Teapot._MAX_TESSELLATE_ATTEMPTS):
            assert Teapot._ensure_tessellated(FINE) is False
        assert Teapot._arrays.get(FINE) == (None, None)      # now latched off
        before = calls['n']
        assert Teapot._ensure_tessellated(FINE) is False
