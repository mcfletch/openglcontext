...
# Force the GLUT backend, but only when this file runs as a standalone script.
# pytest imports every test_*.py at collection time; setting this at module scope
# would leak OPENGLCONTEXT_BACKEND=glut into the shared process environment (and
# into every subprocess the suite later spawns via os.environ.copy()), where
# freeglut cannot open a window under Wayland and every script then fails.
