...
that handoff -- request, poll, supersession, failure -- and the screenshot request,
...
def test_request_screenshot_queues_capture():
    """The screenshot key only sets a flag + asks for a redraw; the picture is
    taken later, by the render pass, with the frame still in the back buffer.
    Where the file lands is `test_default_key_bindings.py`."""
    ctx = make_ctx()
