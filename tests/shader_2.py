...
class TestContext( BaseContext ):
    """This shader passes a per-vertex colour from an input array to
    ...
    def OnInit( self ):
        ...
        Each vertex now carries a colour of its own, declared as a
        second "in" variable beside the position.  The two are read from
        the same buffer, a few bytes apart, which we will describe when
        we draw.

        To communicate the colour for each vertex to the fragment
        shader, the vertex shader declares an "out" variable and the
        fragment shader declares an "in" variable of the same name and
        type.  The value is interpolated across the triangle for each
        fragment, taking the perspectivally correct blended value for
        the vertices which make up each corner.  Thus if we were to
        ...
        You will note that these are declared *outside* the main
        function.  They are the shader's interface to the stages either
        side of it, rather than working values inside one call.
        '''
        ...
        '''Our fragment shader declares the same vertex_color, this
        time as an "in", and one output to write the pixel to.  Since we
        would like the final fragment colour to be the interpolated
        colour between our vertices, we can pass it straight through.'''
        ...
        '''As before, we ask the program where each name it declares
        lives, and keep the answers to use when we draw.'''
        ...
        '''And the vertex array object which will remember how the two
        attributes are read out of the buffer.'''
    ...
    def Render( self, mode):
        ...
        try:
            ...
            try:
                ...
                to the shader, we enable two attribute arrays, one for
                each of the "in" variables the vertex shader declares.

                These "enables" tell OpenGL that for each vertex we
                render, we would like to read one record from the
                enabled arrays.  A location which is not enabled holds
                one fixed value for every vertex instead.
                '''
                ...
                definition calls are:
                    ...
                    * location -- which of the shader's inputs this
                        array feeds
                    * size -- number of values in each record
                    * type -- constant defining the type of value for
                        each item in the record
                    * normalized -- whether integer values should be
                        scaled into 0..1; GL_FALSE for floats, which
                        arrive as they are
                ...
                The position array is passed a reference to our VBO,
