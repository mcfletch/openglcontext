...
class TestContext(BaseContext):
    ...
    def OnInit(self):
        ...
        line = arange(0.0, 1.0, 0.01)
        line2 = line[::-1]
        self.coordinate = Coordinate(
            point=list(zip(line, [0] * len(line), [0] * len(line))),
        )
        self.color = Color(
            color=list(zip(line, [0] * len(line), line2)),
        )
        self.sg = sceneGraph(
            children=[
                Transform(
    ...
    def OnTimerFraction(self, event):
        ...
        yes = sin(range)
        points = list(zip(xes, yes, [0.0] * len(xes)))
        colors = list(zip(xes, xes[::-1], [0.0] * len(xes)))
