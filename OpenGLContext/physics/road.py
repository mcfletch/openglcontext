...
from OpenGLContext.scenegraph.road import (
    RoadProfile, banked_sections, road_surface, sweep_frames, widened_sections,
)
...
class RoadColliders:
    ...
    def __init__(self, world: "PhysicsWorld", points: Any,
        ...
        #: of one width.
        self.widening: Optional[np.ndarray] = self._along(widening, 'widenings')
        ...
        #: Triangles held per chunk, so dropping one takes its own back.
    ...
    def _add(self, key: int) -> None:
        ...
        bodies = []
        for where, triangles in pieces:
            shape = self.world.add_shape(model.Shape.trimesh(where, triangles))
        ...
        self._counts[key] = sum(len(triangles) for _where, triangles in pieces)
        self._triangles += self._counts[key]
    ...
    def _walls(self, first: int, last: int, key: int,
        ...
        for low, high in self.barriers:
            ...
            over = None if lean is None or self.bank is None \
                else self.bank[begin:end + 1]
    ...
    def _span_of(self, values: Optional[np.ndarray], first: int, last: int,
        ...
        Closed exactly where the run of points is: a line already written with
        its first point at the end needs nothing added, and adding it anyway
        would put a figure against a ring that is not there.
        """
        if values is None:
            return None
