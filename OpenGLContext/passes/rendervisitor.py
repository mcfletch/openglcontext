...
class RenderVisitor(visitor.Visitor):
    ...
    def SceneGraphCamera(self, node):
        ...
        if self.viewpointPaths:
            ...
            view = None
            view_path = None
            # moveTo() moves the platform to view_path[-1], so we must keep the path
            # whose leaf is the bound viewpoint -- not just any leftover loop path,
            # or every viewpoint would resolve to the last one scanned.
        ...
        if not self.frustum:
            ...
            log.warning(
                """Calculated frustum twice for the same rendering pass %s""", self
            )
    ...
    def Rendering(self, node):
        ...
        assert hasattr(node, "Render"), (
            """Rendering node %s does not have a Render method""" % (self.__class__)
        )
    ...
    def Transform(self, node):
        ...
        """
        assert hasattr(node, "transform"), (
            """Transforming node %s does not have a transform method"""
            % (node.__class__)
        )
        if self.transform:
    ...
    def ContextSetupBindables(self, node):
        ...
        if not node.getSceneGraph():
            ...
            if not self.frustum:
                ...
                log.warning(
                    """ContextSetupBindables called twice for the same rendering pass %s""",
                    self,
                )
