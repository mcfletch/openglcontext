...
class TestAtlas( unittest.TestCase ):
	...
	def test_matrix( self ):
		...
		bottom_left = dot( array( [0,0,0],'f'), matrix )
		assert allclose( bottom_left, [0,0,0] ), bottom_left
		top_right = dot( array( [1,1,0],'f'), matrix )
		assert allclose( top_right, [.25,.25,0] ), top_right
