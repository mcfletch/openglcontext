...
#: The near plane of a placed camera, as a fraction of the smaller of the world
#: it is in and the distance it is looking, and the closest it is ever put.
NEAR_PLANE_FRACTION = 0.01
...
def look_from(eye: Sequence[float], target: Sequence[float], radius: float,
    ...
    **The near plane is a hundredth of whichever is smaller: the world, or the
    distance being looked at.** A near plane taken from the world alone clips
    away the fourteen metres of road in front of a car standing in a
    four-kilometre dataset; one taken from the aim alone throws away the depth
    precision an interior needs. Whichever is smaller is the one that would
    clip something. The far plane comes from the radius, and reaches at least
    what is aimed at, because whatever you are looking at, the rest of the
    ...
    return CameraPose(
        ...
        orientation=None,
        fov=fov,
        near=max(NEAREST_NEAR_PLANE, min(radius, length) * NEAR_PLANE_FRACTION),
