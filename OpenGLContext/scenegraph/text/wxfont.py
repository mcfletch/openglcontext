...
class wxBitmapFont(font.NoDepthBufferMixIn, font.BitmapFontMixIn, font.Font):
    ...
    def __init__(
        ...
        if not font:
            if __debug__:
                log.info(
                    """wx.BitmapFont passed a null wxPython font, doing lookup for fontStyle %r""",
                    fontStyle,
                )
            family, face, font = wxFontProvider.match(fontStyle)
            if not font:
                raise ValueError(
                    """Could not generate wx.BitmapFont for fontStyle %s"""
                    % (fontStyle,)
                )
        self.font = font
    ...
    def createChar(self, char, mode=None):
        ...
        if dataArray is not None:
            ...
            log.warning(
                """wx.BitmapFont couldn't get display list for character %r""", char
            )
        return None, metrics
    ...
    def textureToList(self, array, metrics, mode=None):
        ...
        try:
            try:
                ...
                if __debug__:
                    log.warning(
                        """Failure creating display-list for %r""", metrics.char
                    )
    ...
    def createCharTexture(self, char, mode=None):
        ...
        data = data[::-1, :, :2]
        assert shape(data) == (height, width, 2), (
            """Data array has changed shape is %s should be %s"""
...
class _wxFontProvider(fontprovider.FontProvider):
    ...
    def calculatePointSize(self, fontStyle, family, face, mode=None):
        ...
        if fontStyle and fontStyle.size:
            targetSize = (
                fontStyle.size * self.scale
            )  # undefined for bitmap fonts in VRML spec
        else:
        ...
        if font is None:
            raise ValueError(
                """Invalid font-size specified (%s), no font found to match that size (target = %s pixels)"""
                % (fontStyle.size, targetSize)
            )
