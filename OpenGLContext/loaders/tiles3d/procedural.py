...
def _fbm(x, z, seed, octaves=5, lacunarity=2.0, gain=0.5):
...
def _ridged(x, z, seed, octaves=5):
    total = np.zeros_like(x, dtype=np.float64)
    amp, freq, norm = 1.0, 1.0, 0.0
    for o in range(octaves):
...
def terrain_colors(positions, normals):
...
def terrain_patch(x0, x1, z0, z1, res, height_fn=None, skirt_depth=0.0):
...
def _add_skirt(pos, nrm, col, idx, res, depth):
...
def build_terrain_tileset(directory, extent=2048.0, levels=3, tile_res=33,
                          height_fn=None):
