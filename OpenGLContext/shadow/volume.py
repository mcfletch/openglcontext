...
class Volume( object ):
	...
	def shadowCaps( self, sourceVector):
		...
		self.forwardIndices = expand(nonzero(greater(directions,0)))

		# backward is trickier, as we need to project to infinity
		# from the light position
		if sourceVector[-1]:
			backwardIndices = expand(nonzero(less_equal(directions,0)))
	...
	def _render_boot( self ):
		...
		if self.sideType != GL_TRIANGLES and self.backwardPoints:
