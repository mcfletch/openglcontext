        ...
        while '\n' in self._buffer:
            ...
            if line:
                try:
                    ...
                    print(f"EventInjector: Invalid JSON: {line[:50]}... ({e})")
...
class EventInjectionMixin:
    ...
    def _dispatch_injected_event(self, event: Dict[str, Any]) -> None:
        ...
        if event_type == 'mousebutton':
            ...
            print(f"EventInjector: Unknown event type: {event_type}")
    ...
    def _inject_mousebutton(self, event: Dict[str, Any]) -> None:
        ...
        # Set pick point
        synth_event._pickPoint = (float(x), float(y), 0.0)

        # Dispatch through event manager
        event_name = 'mousebutton'
        manager = self.getEventManager(event_name)
    ...
    def _do_injection_capture(self, name: str) -> None:
        ...
        print(f"CAPTURE: {name} ({capture.width}x{capture.height})")
