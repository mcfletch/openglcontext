...
def run_test(
    ...
    Thin alias for :func:`run_test_with_popen`. The historical implementation
    used ``subprocess.run``, whose ``TimeoutExpired`` exposes no PID, so on
    timeout it could only kill the direct child -- orphaning the GL grandchild
    (zombie GLFW/GLUT windows on CI). The Popen path knows the
    child PID and tears down the whole tree, so both entry points now share it.
